...
include_once __DIR__ . '/BillPTSync.php';
include_once __DIR__ . '/Api.php';
include_once __DIR__ . '/BillPT.php';

add_action('admin_notices', 'bill_pt_success_message');

function bill_pt_createDefaultDB()
{
    global $wpdb;
    ...
    $wpdb->query("CREATE TABLE IF NOT EXISTS `bill_produtos_sync`(
        id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
        date TIMESTAMP,
        value MEDIUMTEXT
        ) ENGINE=MyISAM  DEFAULT CHARSET=utf8 AUTO_INCREMENT=2 ;");

    $wpdb->query("CREATE TABLE IF NOT EXISTS `bill_stock_sync`(
        id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
        date TIMESTAMP,
        value VARCHAR(60)
        ) ENGINE=MyISAM  DEFAULT CHARSET=utf8 AUTO_INCREMENT=2 ;");

    $wpdb->query("CREATE TABLE IF NOT EXISTS `bill_sync_log`(
            id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
            date TIMESTAMP,
            type VARCHAR(20),
            value MEDIUMTEXT
...
function bill_pt_activate_bill()
{
    ...
    $wpdb->insert('bill_config', [
        'config' => 'default_config', 'value' => json_encode($config)
    ], ['%s', '%s']);
}
...
function bill_deactivate_bill()
{
    ...
    #   $wpdb->query("DROP TABLE bill_produtos_sync");
    #   $wpdb->query("DROP TABLE bill_stock_sync");
    #   $wpdb->query("DROP TABLE bill_sync_log");
}
...
if (bill_pt_woocommerce_in_the_house()) {
    ...
    add_action('plugins_loaded', 'bill_pt_woocommerce_loaded');

    function bill_pt_woocommerce_loaded()
    {
        add_action('wp_loaded', 'bill_pt_last_thing');
    }

    function bill_pt_last_thing()
    {
        if (isset($_GET['key_stock_cron'])) {
            $bill = new BillPT(BILL_API_MODE);
            $bill->login();
            if ($bill->isLogged()) {
                $sync = new BillPTSync($bill->getApi());
                $sync->syncStock();
            }
            die("finish");
        }

        if (isset($_GET['key_products_cron'])) {
            require_once WP_PLUGIN_DIR . '/woocommerce/includes/import/class-wc-product-csv-importer.php';
            $bill = new BillPT(BILL_API_MODE);
            $bill->login();
            if ($bill->isLogged()) {
                $sync = new BillPTSync($bill->getApi());
                $sync->syncProductList();
                $sync->importUpdateProducts();
            } 
            die("finish");
        }
    }
    ...
    add_action('save_post_product', 'bill_pt_on_product_update_or_create', 10, 3);
    function bill_pt_on_product_update_or_create($post_id, $post, $update)
    {
        $sync = new BillPTSync(null);
        $sync->updateSkuToID();
    }
    ...
    function bill_pt_display_settings()
    {
        ...
        $woo_bill = new BillPTSync($bill->getApi());
        #update formularios
        $woo_bill->updateProductConfig();
        $woo_bill->updateStockConfig();
        $woo_bill->updateCustomMap();

        if ($bill->isLogged()) {
            if (is_admin() && isset($_GET['create_extra_fields'])) {
                $woo_bill->checkIfWooMetaExist($_GET['create_extra_fields']);
            }
            $bill->updateConfig();
            $bill->createDocument();
            $bill->sendEmail();
